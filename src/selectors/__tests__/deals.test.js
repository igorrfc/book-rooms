...
describe('deals selectors', () => {
  describe('filteredDeals', () => {
    const offerWithBreakfast = {
      availableRooms: 2,
      bookURI:
      ...
      id: 'e9c3f14241544d25cb53aa14cf2dc2efaa42268e5c41302b5ca4c17c14680b1c:00',
    ...
    const offers = [payLaterOffer, offerWithBreakfast, offerWithWifi];

    describe('when breakfast filter is enabled', () => {
      it('returns only offers with breakfast included', () => {
        const state = {
          dealFilter: { data: DealFilter.Breakfast },
          deals: {
            data: {
              offers,
            },
        ...
        expect(filteredDeals(state)).toEqual([offerWithBreakfast]);
      });
    });

    describe('when free wifi filter is enabled', () => {
      it('returns only offers with free wifi', () => {
        const state = {
          dealFilter: { data: DealFilter.FreeWifi },
          deals: {
            data: {
              offers,
            },
        ...
        expect(filteredDeals(state)).toEqual([offerWithWifi]);
      });
    });

    describe('when pay later filter is enabled', () => {
      it('returns only offers with pay later as an option', () => {
        const state = {
          dealFilter: { data: DealFilter.PayLater },
          deals: {
            data: {
              offers,
            },
        ...
        expect(filteredDeals(state)).toEqual([payLaterOffer]);
      });
    });
  });
